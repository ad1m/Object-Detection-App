...
def run_model(image_name):
    ...
    TEST_IMAGE_PATHS = [ os.path.join(PATH_TO_TEST_IMAGES_DIR, image_name)]
...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    image.save('static/images/image1d.jpg','JPEG')
...
def draw_keypoints_on_image(image,
  ...
  image.save('../../detected_images.jpg')
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh and classes[i] == 1: #Here we do a classes[i] == 1 check to detect only people
